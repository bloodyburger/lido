...
def upsert_to_postgresql(data_frame, table_name, schema=None, match_columns=None):
    ...
    engine = sa.create_engine(
        os.environ.get("CONNECTION_STRING"), isolation_level="AUTOCOMMIT"
    )
    table_spec = ""
    if schema:
...
def process_snowflake():
    ...
    df_dune_Data = df_dune_Data.drop_duplicates(
        subset="HASH_KEY", keep="first", inplace=False
    )
    print(
        "No of records from after removing duplicates is {}".format(
            df_dune_Data.shape[0]
        )
    )
    ...
    upsert_to_snowflake(
        df_dune_Data, id_columns, insert_columns, update_columns, table_name, stage_name
    )
...
def process_postgresql(df_dune_Data):
    ...
    # Include hash column
    df_dune_Data["hash_key"] = (
        df_dune_Data["period"].astype(str)
        + df_dune_Data["primary_label"].astype(str)
        + df_dune_Data["secondary_label"].astype(str)
        + df_dune_Data["account"].astype(str)
        + df_dune_Data["category"].astype(str)
        + df_dune_Data["subcategory"].astype(str)
        + df_dune_Data["base_token_address"].astype(str)
        + df_dune_Data["hash"].astype(str)
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Pull data from Dune and write to Snowflake"
        )

        # Add command line arguments
        parser.add_argument(
            "--days", type=int, help="No of days of data to pull from Dune"
        )
        ...
        print(
            "** Script execution completed at {}".format(
        ...
        send_email(
            subject,
            "Data loaded from Dune to Postgresql successfully for date range >= {} and inserted {} records".format(
                date_to_pull, df_dune_Data.shape[0]
            ),
            recipients,
        )
        ...
        send_email(
            subject,
            str("".join(traceback.TracebackException.from_exception(e).format())),
            recipients,
        )
